                  ...
                  {(firstTier[v].number || []).map((e, i) => {
                    return (
                      <Col key={e} xs={12} sm={8} lg={5}>
                        <Text type={styles[firstTier[v].id].textType}>{e}</Text>
                      </Col>
                    );
                  })}
                </Row>
...
function Random1() {
  ...
  const mostFrequent = Object.keys(numbers).sort(
    (a, b) => numbers[b] - numbers[a]
  )[0];
  const lastFrequent = Object.keys(numbers)
    .sort((a, b) => numbers[b] - numbers[a])
    .last();
  return (
    <Title>
      1 prediction number {mostFrequent} {numbers[mostFrequent]}{" "}
      {numbers[lastFrequent]}
    </Title>
  );
}
