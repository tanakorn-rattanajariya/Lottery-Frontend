...
export default function SearchResult(props){
    ...
    const checkResult=(value)=>{
        ...
        setWinningPrizes((result?.prizes||[])
        .filter(v=>(v.number||[]).filter(e=>e===value).length>0)
        .reduce((acc, v)=>{return {...acc,[v.id]:v}},{}));

        setWinningRunning((result?.runningNumbers||[])
        .map(v=>{
            var length = value?.length;
            switch(v.id){
                case "runningNumberFrontThree":
                    if((v.number || []).filter(e=>e===value.toString().substring(0,3)).length>0){
                        return v;
                    }
                    return;
                case "runningNumberBackThree":
                    if((v.number || []).filter(e=>e===value.toString().substring(length-3,length)).length>0){
                        return v;
                    }
                    return;
                case "runningNumberBackTwo":
                    if((v.number || []).filter(e=>e===value.toString().substring(length-2,length)).length>0){
                        return v;
                    }
                    return;
                default:
        ...
        <div style={{marginTop:'16px'}}>
            <Input.Search onSearch={(e)=>checkResult(e)}/>
            {
                winningPrizes && winningRunning ? 
                    Object.keys(winningPrizes).length>0 || Object.keys(winningRunning).length>0?
                    <div>
